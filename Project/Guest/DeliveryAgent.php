...
if (isset($_POST['btn_submit'])) {
    ...
    move_uploaded_file($path, "../Assets/Files/DeliveryBoy/" . $photo);

    $proof = $_FILES['file_proof']['name'];
    $path1 = $_FILES['file_proof']['tmp_name'];
    move_uploaded_file($path1, "../Assets/Files/DeliveryBoy/" . $proof);

    $insQry = "INSERT INTO tbl_delivery
        (delivery_name, delivery_email, delivery_address, delivery_password, delivery_proof, delivery_photo, delivery_contact, place_id)
        VALUES ('$name','$email','$address','$password','$proof','$photo','$contact','$place')";
    if ($Con->query($insQry)) {
        echo "<script>alert('Delivery Agent Registered Successfully'); window.location='Login.php';</script>";
    } else {
...
<title>AfterBump :: Delivery Agent Registration</title>
...
<div class="registration-card">
  <!-- Left Side (Form) -->
  <div class="reg-left">
    <h3><i class="fas fa-bicycle"></i> Join AfterBump as a Delivery Agent</h3>
    <form method="post" enctype="multipart/form-data">
      ...
      <div class="form-group">
        <label for="delivery_district" class="form-label">Select District</label>
        <select name="delivery_district" id="delivery_district" class="form-select" onchange="getplace(this.value)" required>
      ...
      <div class="form-group">
        <label for="delivery_place" class="form-label">Select Place</label>
        <select name="delivery_place" id="delivery_place" class="form-select" required>
